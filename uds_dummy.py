...
def sendRequest(request, IsPosResExpected = True):
    ...
    if(request[0] == 0x22): #dummy implementation for service 22
        ...
        elif(request[1:] == [0x10, 0x04]):
            res = [0x62, 0x10, 0x04, 0x23]
            #res = [0x62, 0x01, 0x03, 0x88, 0x6C, 0x5E, 0x3B]
            response.resp = res.copy()
        ...
        if(request[1] == 0x01 and request[2]==0x08):
            ...
            response.nrcname = NRC_DATA.get(response.nrc, "Unknown NRC")[0]
            response.nrcdesc = NRC_DATA.get(response.nrc, "Unknown NRC Recieved. There is no record of this NRC in UDS ISO 14229 Document")[1]
        elif(request[1] == 0x01 and request[2]==0x88):
            res = [0x59, 0x01, 0x2F, 0x01, 0x00, 0x01]
            response.resp = res.copy()
            response.type = "Positive Response"
            response.nrc = 0x00
            ...
            res = [0x59, 0x0C, 0x7F, 0x12, 0x34, 0x56, 0x24, 0x23, 0x45, 0x05, 0x00, 0xAB, 0xCD, 0x01, 0x2F]
            response.resp = res.copy()
            response.type = "Positive Response"
            response.nrc = 0x00
            response.nrcname = NRC_DATA.get(response.nrc, "Unknown NRC")[0]
            ...
            res = [0x59, 0x0D, 0x7F, 0x12, 0x34, 0x56, 0x24, 0x23, 0x45, 0x05, 0x00, 0xAB, 0xCD, 0x01, 0x2F]
            response.resp = res.copy()
            response.type = "Positive Response"
            response.nrc = 0x00
            response.nrcname = NRC_DATA.get(response.nrc, "Unknown NRC")[0]
            ...
            res = [0x59, 0x0E, 0x7F, 0x12, 0x34, 0x56, 0x24, 0x23, 0x45, 0x05, 0x00, 0xAB, 0xCD, 0x01, 0x2F]
            response.resp = res.copy()
            response.type = "Positive Response"
            response.nrc = 0x00
            response.nrcname = NRC_DATA.get(response.nrc, "Unknown NRC")[0]
        ...
        if(request[1] == 0x11):
            ...
            if(request[2]==0x22):
                if(request[3:5]==[0x40,0x00]):
                    if(request[5:]==0x65,0x00):
                        res = [0x74,0x20,0x0f,0xff]
                        response.resp = res.copy()
                        response.type = "Positive Response"
                        response.nrc = 0x00
                        response.nrcname = NRC_DATA.get(response.nrc, "Unknown NRC")[0]
    ...
    elif(request[0] == 0x36): #dummy implementation for service 34
        print("hrere1")
        if(request[1] == 0x11):
            print("hrere2")
            res = [0x7F, 0x28]
    ...
    elif(request[0:3]==[0x1f,0xff,0x36]):
            response.type="Positive Response"
            res=[0x30,0x00,0x00]
            response.resp=res.copy()

    elif(request[0] == 0x3E): #dummy implementation for service 3E
        if(request[1] == 0x00):
            res = [0x7E, 0x00]
            response.resp = res.copy()
            response.type = "Positive Response"
