...
from service19_0D_base import Ui_Form_SID_19_0D
...
class Ui_Service19_0D(Ui_Form_SID_19_0D):
    def redesign_ui(self):
        pass

    def connectFunctions(self):
        self.pushButton_Send19_0DReq.clicked.connect(self.send19_0Dservice)
    ...
    def clearform(self):
        ...
        gen.log_action("Button Click", "Clear Form for Service 19 subfunction 0D window clicked. Userfields cleared successfully.")
        return
    ...
    def addlog(self):
        ...
        gen.log_action("Button Click", "Add to Log button for Service 19 subfunction 0D window clicked.")
        return
    
    def clearlog(self):
        gen.clearudslogfile()
        self.update_status(f"Log file cleared (./reports/uds_log_report.txt)")
        gen.log_action("Button Click", "Clear Log button for Service 19 subfunction 0D window clicked.")
        return
    
    def send19_0Dservice(self):
        sprmib_flg = self.checkBox_suppressposmsg.isChecked()
        service_request = fun.form_reqmsg4srv19_subfun_D(sprmib_flg)
        ...
        if(response.type == "Positive Response"):
            self.update_status("Service 19 subfunction 0D response is received")
    ...
    <p><strong>Subfunction Name:</strong> <I>Report Most Recent Test Failed DTC {hex(response.resp[1]).upper()} </I></p>
    ...
    <p><strong>Info:</strong> <I> Service 19 sunfunction 0D is successfully executed</I></p>
'''
...
'''
 ...
 Explaination:   Read DTC Information (Service 19 subfunction 0D) Requested 
...
Subfunction Name: Report Most Recent Test Failed DTC {hex(response.resp[1])}
Suppress Positive Message Request: {sprmib_flg}
DTC Status Availability Mask: {hex(response.resp[2])}
{dtc_records_text}
Info: Service 19 Subfunction 0D was successfully executed.
<------------------- LOG ENTRY END ------------------->
...
# '''
    ...
    def closeEvent(self, event):
        ...
        gen.log_action(f"Window Close", f"Service 19 0D Window Closed.")
        return
...
if __name__ == "__main__":
    ...
    Form_SID19_0D = QtWidgets.QWidget()
    ui = Ui_Service19_0D()
    ui.setupUi(Form_SID19_0D)
    ui.redesign_ui()
    ui.connectFunctions()
