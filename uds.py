...
def sendRequest(request, IsPosResExpected = True):
    ...
    if (status == True):
        res = tp.recieve_data()
        if(res == False):   #when no response is recieved
            if(IsPosResExpected == False):  #Positive response is not expected and neg res also not recieved.
                response.type = "No Response"
                response.nrc = 0x00
                response.nrcname = "No NRC"
                response.nrcdesc = "There is no Negative Response Code"
                response.resp = []
                return(response)
            else:
                response.type = "No Response"
                ...
                response.nrcdesc = "There is no Negative Response Code"
                response.resp = []
                print(f"Error: No response recieved till timeout even when response was expected.")
                return(response)
        ...
        while((res[0] == 0x7F) and (res[2] == 0x78)):   #If positive response pending is recieved then
            ...
            if(res[0] == 0x7F): #If the negative response is recieved
            ...
            elif(res[0] == (0x40 | request[0])):    #If the positive response is recieved
    ...
    else: #When senddata did not return status as True
        print(f"The request of UDS service failed")
        return ("Error")    
