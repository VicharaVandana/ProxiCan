...
def send_big_data(n_sdu):  #For big data more than 7 bytes up to 4095 bytes as specified by ISO15765 protocol
    ...
    pending_datalength = datalength - 6 #6 bytes of N_SDU is sent in first frame
    pending_data = n_sdu[5:]
            ...
            if(flowParameters != None):
                ...
                if (flow_status == 0):   #Continue To Send Consecutive frames
                    ...
                    while((pending_datalength > 0) and (cf_count <= block_size)):    #Either databytes are over or block size is crossed till then do this
                        seq_num = cf_count % 16
                        current_chunk = pending_data[:7]    #The data chunk which will be sent in the current consecutive frame
                        send_consecutiveframe(seq_num,current_chunk,cf_gap_time_min)    #Send the consecutive frame
                        pending_data = pending_data[7:] #Pending N_SDU Data to be transmitted
                        cf_count = cf_count + 1
                        total_cf_count = total_cf_count + 1
                        pending_datalength = pending_datalength - 7 #7 bytes sent in one consecutive frame max.
...
def recieve_data():
    ...
    if received_message:    #If a Message is recieved from ECU side with Diag Response CAN ID within the timeout period
        ...
        if(frametype == 0x00):  #Single frame recieved
            ...
            print(f'Single Frame Recieved. N_SDU = [{" ".join(hex(number) for number in n_sdu_rx)}]')
            return(n_sdu_rx)
            ...
            print(f"Expected zSF or FF but Invalid Frame recieved in the flow with frame type = {frametype} and the whole data = [{' '.join(hex(number) for number in data)}]")
            return False
