...
class SecuLvlExportTool(QWidget):
    ...
    def init_ui(self):
        ...
        # Export Section
        layout.addWidget(QLabel("Export Configuration"))
        ...
        self.line_edit_export_path = QLineEdit(self)
        self.line_edit_export_path.setPlaceholderText("Enter or browse the file path to export...")
        layout.addWidget(self.line_edit_export_path)

        self.button_browse_export = QPushButton("Browse Export Path")
        self.button_browse_export.clicked.connect(self.browse_export_file)
        layout.addWidget(self.button_browse_export)
    ...
    def browse_export_file(self):
        file_path, _ = QFileDialog.getSaveFileName(self, "Choose Export File", "", "Encoded Files (*.enc);;All Files (*)")
        if file_path:
            self.line_edit_export_path.setText(file_path)

    def export_selected_items(self):
        file_path = self.line_edit_export_path.text()
        ...
        try:
            ...
            json_string = json.dumps(self.selected_items)
            encoded_data = self.cipher_suite.encrypt(json_string.encode())

            with open(file_path, 'wb') as export_file:
                export_file.write(encoded_data)
